...
class Notification(models.Model):
    ...
    """
    user = models.ForeignKey(User, related_name="notifications", verbose_name=_(u"user"))
    created_at = models.DateTimeField(auto_now_add=True, verbose_name=_(u"date created"))
    checked_at = models.DateTimeField(blank=True, null=True, verbose_name=_(u"read at"))
    # We can pass arbitrary keyword to simplify notification management
    key = models.CharField(max_length=32, blank=True, default="", verbose_name=_(u"keyword"))
    # This will be part of human-friendly description
    action_verb = models.CharField(max_length=64, blank=True, null=True, verbose_name=_(u"verb"))
    action_actor = models.ForeignKey(User,
        related_name="made_notifications",
        blank=True, null=True, verbose_name=_(u"actor"))
    # This will work like similar fields in activity stream
    # We take some object involved in action...
    action_object_ct = models.ForeignKey(ContentType, blank=True, null=True,
        related_name="notify_action_objects")
    action_object_id = models.PositiveIntegerField(blank=True, null=True)
    action_object = generic.GenericForeignKey('action_object_ct', 'action_object_id')
    # ...and it's target
    action_target_ct = models.ForeignKey(ContentType, blank=True, null=True,
        related_name="notify_action_targets")
    action_target_id = models.PositiveIntegerField(blank=True, null=True)
    action_target = generic.GenericForeignKey('action_target_ct', 'action_target_id')
